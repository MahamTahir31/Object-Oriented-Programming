...
public:
    ...
    {
        n_kinds = 5;
        cout <<"There are total "<<n_kinds<<" kinds which we can discuss in this applicationn from which, one is given below:\n" << endl;
    }
    void details()
    {
    ...
    friend ostream &operator<<(ostream &out, Humans &obj){
        out << obj.n_kinds << endl;
    }
    friend istream &operator>>(istream &in, Humans &obj);
};

class normal : public Humans // FIRST CHILD CLASS
{
public:
    normal() : Humans() // inheritance of derived class CONSTRUCTOR with base class CONSTRUCTOR
    {
        Speak = "can speak";
        See = "can see";
        Hear = "can hear";
